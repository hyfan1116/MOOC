...
def sense(p, world,Z,sensor_right):
	q = [[0.0 for x in range(0,len(p[0]))] for y in range(0,len(p))]
...
def localize(colors,measurements,motions,sensor_right,p_move):
	...
	if len(measurements) != len(motions):
		raise ValueError, "Error in size of vector"
		
    for step in range(0,len(motions)):
